...
class QuestionUsecaseTest(TestCase):
    def test_run(self):
        self.assertEqual(
            isinstance(create_question(logger=MagicMock()),
                       CreateQuestion), True)

        self.assertEqual(
            isinstance(update_question(logger=MagicMock()),
                       UpdateQuestion), True)

        self.assertEqual(
            isinstance(find_question(logger=MagicMock()),
                       FindQuestion), True)

        self.assertEqual(
            isinstance(get_question_list(logger=MagicMock()),
