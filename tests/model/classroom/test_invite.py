...
class ClassmateInviteTest(TestCase):
    ...
    def test_create(self):
        invite = ClassmateInvite.create(classroom_id=ClassroomId(1))
        self.assertEqual(type(invite.invite_key.value), str)
        self.assertEqual(invite.classroom_id.value, 1)
        self.assertEqual(invite.expire_date.to_string(),
                         '2020-02-15T00:18:16.874000+09:00')

    def test_dict(self):
        invite = ClassmateInvite.from_dict(self.invite_dict)
        self.assertEqual(invite.to_dict(), self.invite_dict)
