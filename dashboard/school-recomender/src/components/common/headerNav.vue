<template>
  <v-container>
    <v-navigation-drawer v-model="drawer" :clipped="$vuetify.breakpoint.lgAndUp" app fixed>
      <v-list dense>
    ...
    <v-app-bar :clipped-left="$vuetify.breakpoint.lgAndUp" app color="yellow darken-1">
      ...
      <v-avatar size="40">
        <v-icon v-if="myAvatarImageUrl == ''">mdi-account</v-icon>
        <v-img v-if="myAvatarImageUrl != ''" :src="myAvatarImageUrl" alt="Cropped Image" />
      </v-avatar>
    </v-app-bar>
    <v-dialog v-model="logoutDialog" width="300px">
      <v-card style="texta">
        <v-card-title>
          <v-spacer />
        ...
        <v-card-actions class="justify-space-around">
          <v-btn @click="logoutDialog = false">キャンセル</v-btn>
          <v-btn color="yellow darken-1" @click="logout">ログアウト</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
